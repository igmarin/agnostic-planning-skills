...
# Agnostic Planning Skills — skill catalog validator
#
# Fails on:
#   - invalid / missing directory.json and skills.sh.json
#   - missing description / name / type
#   - name ≠ directory
#   - folded description > 600 chars
#   - SKILL.md > 500 lines
#   - directory.json ↔ disk sync
#
# Warns (does not fail) on missing body sections.
...
# Prefer /usr/bin/python3 (system) to avoid version-manager shims (mise, asdf,
# pyenv) that may hang on startup. Fall back to python3 on PATH for Linux/CI.
...
while IFS= read -r skill_file; do
  ...
  skill_name=$(basename "$(dirname "$skill_file")")
  [ "$skill_file" = "SKILL.md" ] && skill_name="agnostic-planning-skills"
...
PY
  ...
  if [ "$desc_len" -lt 0 ]; then
    ...
    check_fail "$skill_name: description is ${desc_len} chars (spec max ${DESC_LIMIT})"
  ...
  if [ "$body_lines" -gt "$BODY_LINE_WARN" ]; then
    check_fail "$skill_name: SKILL.md is ${body_lines} lines (limit ${BODY_LINE_WARN})"
  else

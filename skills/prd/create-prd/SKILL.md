...
description: >
  ...
  surface, success metrics, and open questions. Language-agnostic.
...
DO NOT include any code, pseudo-code, SQL, class names, or method signatures in the PRD.
...
3. **Draft** — fill **[PRD_TEMPLATE.md](./PRD_TEMPLATE.md)** section by section (canonical order). Do not invent a parallel outline.
...
- Mention frameworks, libraries, or platform choices only when they **constrain scope** (auth, jobs, timeouts, conventions), not as step-by-step implementation.
- Call out effects on **middleware, callbacks, or background workers** in **Design and Technical Considerations** or **Non-Functional Requirements** when relevant.
...
3. **Functional Requirements language** — Written in natural language ("the system must send a confirmation email when…"), not code.
4. **Next Steps** — Close the PRD with the suggested follow-on (typically: "Run `generate-tasks` against this PRD once approved.").
5. **No Code** — Never include code, pseudo-code, SQL, class names, or method signatures.

...
This template defines the standard structure for all Agnostic Planning Skills agents. Use this when creating new agents or refactoring existing ones.
...
function applyPromo(price, percentage) { ... }
...
```text
# Before: Single module handles multiple responsibilities
...
1. Choose the best first failing test
2. Write the test and verify it FAILS for the right reason
...
1. Check configuration for database credentials

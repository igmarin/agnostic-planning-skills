...
2. **Tool-Agnostic & Portable:**
  - **Focus on Principles, Not Tools**: Generalize specific tool names (e.g., `linter` instead of a specific tool). Exceptions are allowed for terms tightly coupled to a community standard.
...
5. **Illustrative Code Examples:**
  ...
  - **Contextual & Copyable**: Ensure examples are realistic for the domain and can be easily copied and adapted.

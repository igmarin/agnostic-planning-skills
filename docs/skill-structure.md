...
- The minimum command sequence (e.g., `<test-command>`, `<doc-tool>`, etc.)
...
Keep steps imperative ("Write the test", "Run `<test-command> path/to/test`") rather than declarative — the skill is an instruction sheet, not documentation of behavior.

## 5. Output Style

The exact shape of every artifact this skill produces. This section ensures consistent output across runs. Include:
...
The validator does **not** yet enforce the presence of HARD-GATE, Output Style, or Integration sections. These are enforced by manual review and consistency checks.
